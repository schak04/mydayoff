...
const Dashboard = () => {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* ── Admin Panel Card ── */}
            {user?.role === 'Admin' && (
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-sm transition-all duration-300">
                    <div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">System Administration</h3>
                        <p className="text-slate-500 dark:text-slate-400">Manage users, roles, and organizational structure.</p>
                    </div>
                    <Link
                        to="/admin"
                        className="bg-primary-600 hover:bg-primary-700 text-white px-6 py-2.5 rounded-lg font-semibold transition-all shadow-lg shadow-primary-500/20"
                    >
                        Access Admin Panel
                    </Link>
                </div>
